...
def load_data():
    ...
    possible_paths = [
        base_dir / "data" / "Cleaned_FAANG_Data-1.xlsx",
        base_dir.parent / "data" / "Cleaned_FAANG_Data-1.xlsx",
        base_dir / "Cleaned_FAANG_Data-1.xlsx"
    ]
    ...
    raise FileNotFoundError(
        "Excel file not found. Make sure "
        "'Cleaned_FAANG_Data-1.xlsx' is inside the GitHub repository."
    )
...
def run_models(df):
    ...
    ] = best_reg_pipeline.predict(
        X_test
    )

    df_test[
        "Predicted_Market_Direction"
    ] = tuned_xgb_cls.predict(
        X_test
    )
...
with tab2:
    ...
    m1.metric(
        "MAE",
        f'{selected_row["MAE ($)"]:.2f}'
    )

    m2.metric(
        "RMSE",
        f'{selected_row["RMSE ($)"]:.2f}'
    )

    m3.metric(
        "R²",
        f'{selected_row["R2 Score"]:.4f}'
    )
...
with tab3:
    ...
    st.write(
        "Best parameters found by TimeSeriesSplit:"
    )
...
with tab4:
    ...
    st.write(
        ...
        "Best regression model selected by "
        "the notebook logic: "
    ...
    st.subheader(
        "Predicted vs Actual Next-Day Price"
    )
...
st.caption(
    ...
    "Model workflow follows the FAANG notebook: "
